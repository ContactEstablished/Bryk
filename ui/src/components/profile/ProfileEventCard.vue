...
// One event's form. `event` present = existing row (Save updates, Delete removes
// server-side); absent = unsaved draft (Save creates, Remove discards locally).
const props = defineProps<{ event?: EventResponse | null }>()
...
function toFormItem(e: EventResponse): EventFormItem {
  return {
    name: e.name,
...
async function onDelete() {
  ...
  try {
    ...
    // The re-fetch drops this row from the list, unmounting the card.
  } catch (e) {
...
<template>
  <div class="rounded-md border p-4 space-y-4">
    <div class="flex items-center justify-between">
      <span class="text-sm font-medium">{{ isDraft ? 'New event' : 'Event' }}</span>
      <Button
        v-if="isDraft"
        type="button"
        variant="ghost"
