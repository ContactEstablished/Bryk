...
// One goal's form. `goal` present = existing row (Save updates, Delete removes
// server-side); absent = unsaved draft (Save creates, Remove discards locally).
const props = defineProps<{ goal?: GoalResponse | null }>()
...
function toFormItem(g: GoalResponse): GoalFormItem {
...
const onSubmit = form.handleSubmit(async (values) => {
  ...
  // Goals editor doesn't expose GoalType; every goal is General (Decision 5).
  const dto: GoalDto = {
    type: 'General',
...
<template>
  <div class="rounded-md border p-4 space-y-4">
    <div class="flex items-center justify-between">
      <span class="text-sm font-medium">{{ isDraft ? 'New goal' : 'Goal' }}</span>
      <Button
        v-if="isDraft"
        type="button"
        variant="ghost"
