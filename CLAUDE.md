...
8. **Plan POST/PUT validator bounds diverge** (Phase 18, deliberate): `TrainingPlanRequestValidator` accepts `BuildWeeks > 0` / `RecoveryWeeks > 0` / `RecoveryWeekPercentage` 0–100, while `TrainingPlanUpdateRequestValidator` bounds them to 1–8 / ≥ 1 / 30–90. Tightening the shipped POST is an API breaking change — **needs a Sr. Dev decision**, deferred three times now (Phases 18, 19, 20).
9. `PUT /api/v1/wellness/{date}` accepts a date-**with-time** segment (`…/2026-07-25T10:00:00` → 200, writing `2026-07-25`): ASP.NET Core's `DateOnly` binder accepts an ISO datetime and truncates it. Harmless — the URL canonicalises to a valid non-future date and no bad date reaches the DB — but a stricter route constraint would make the contract exact (Phase 20).
10. `HrvCard.vue` renders "1 days logged" — no singular/plural handling, unlike `SleepCard`. The string is pinned by `Tasks-20-4.md`; cosmetic one-line fix.
11. `ui/src/components/dashboard/PlaceholderCard.vue` is unused since Phase 20 removed its only import. Kept deliberately (surgical-changes rule); delete in a standalone cleanup.
...
- Current phase: **Phase 20 complete** (wellness metrics — ADR-0011 made executable: `DailyWellness` (the one approved migration, one wide row per athlete per day, every metric nullable, unique composite index on `{AthleteId, Date}`, **no FK and no nav to `Athlete`**), a service-side read-then-update upsert behind `PUT /api/v1/wellness/{date:datetime}` (200 for create *and* update, whole-day replace), `GET /wellness?from=&to=` mirroring the analytics range contract, `GET /wellness/summary` carrying 7-day averages + prior-week deltas + a sparse 14-day series in one call, a pure `WellnessSummaryCalculator`, the shared **`ScaleSelector`** that `RpeSelector` now wraps, a Today quick-entry card, and real Sleep/Weight/HRV tiles plus a Resting HR trend with a read-only profile fallback; see `md/handoffs/2026-07-26-phase-20-complete.md`). **Zero new packages.** The date parameter is guarded twice on purpose — a `{date:datetime}` route constraint (malformed → 404) *and* a validator `default` guard (→ 400) — because `Program.cs:32–33` disables the automatic model-state 400. Next: **Phase 21** — production hardening & deployment (ProblemDetails, observability, containerization, tech-debt burn-down). **Phase 12** — Authentication & Authorization — remains deferred and **approval-gated** (see Open decisions). Phases 8–11 and 13–19 are complete.
- ADRs (`/md/decisions/`) — read before touching the training/zone domain:
  ...
  - **0010** — Activity file import: `Garmin.FIT.Sdk` 21.205.0 approved for `Bryk.Infrastructure` only (proprietary royalty-free FIT licence, not OSI), `.tcx`/`.gpx` on `System.Xml.Linq`; raw bytes in the DB as `varbinary(max)` behind a ~25 MB **per-route** cap (never global Kestrel config); imported power/pace reach the load math through **one synthetic `WorkoutStepResult`** (`WorkoutStepId = null`, `OrderIndex = 0`) — `LoadCalculator` unchanged, and no `Workout.AvgPower`/`AvgPace`; migration limited to **`ActivityFile` alone** (no `Workout.SourceFileId`, no `WorkoutZoneDuration` — the "from file" badge and the duplicate-commit guard both read the reverse link `ActivityFile.ParsedWorkoutId`); the per-zone histogram is **JSON on the same row**, reported as method `samples` and taking precedence over structure/sessionAvg (normalizing it is Phase-21 debt); no per-second sample persistence (Accepted).
  - **0011** — Wellness metrics: `DailyWellness` is **independent of `Athlete`** — a wellness save never writes back to `Athlete.WeightKg`/`RestingHr` (verified: neither feeds any load, zone or PMC math), the single concession being a **read-only** Resting HR tile fallback with **no equivalent for weight**; one wide mostly-nullable row per athlete per day, uniqueness enforced by a composite index **and** a service-side read-then-update upsert (the EF InMemory test provider enforces no unique index, so **no test may assert a duplicate insert throws** — the index is verified by reading the migration, the behaviour by counting rows through the API); PUT replaces the whole day and there is **no DELETE** in v1; HRV does **not** blend into TSB/PMC or any readiness score (ADR-0006 stays pure); the soreness/sleep-quality input **generalizes** `RpeSelector` into a shared `ScaleSelector` (literal `grid-cols-5`/`grid-cols-10` strings — an interpolated class is invisible to Tailwind's scanner); `DeltaChip` is **not** recoloured — inverted metrics (RHR, weight, soreness) report their change in `MetricTile`'s footer slot; one migration, `DailyWellness` alone, zero new packages (Accepted).
