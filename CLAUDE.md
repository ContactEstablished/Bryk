...
- Current phase: **Phase 19 complete** (activity file import — ADR-0010 made executable: `ActivityFile` (the one approved migration, `varbinary(max)` bytes + `ParsedWorkoutId` reverse link + `ZoneHistogramJson`), an Application-side `IActivityFileParser` boundary with TCX/GPX on `System.Xml.Linq` and FIT on `Garmin.FIT.Sdk` 21.205.0 (the one approved package, `Bryk.Infrastructure` only), pure `ZoneHistogramCalculator`, a two-step `POST /api/v1/activityfiles` → `POST .../{id}/commit` flow with match candidates, a 25 MB validator cap behind a 32 MB per-route attribute, the **synthetic `WorkoutStepResult`** that routes imported power/pace into `LoadCalculator` with the calculator frozen, an import review flow on `WorkoutsView` + a "from file" badge from the reverse lookup, and `samples`-method time-in-zone taking precedence over the estimate chain; see `md/handoffs/2026-07-26-phase-19-complete.md`). Two carry-forwards: the committed `.fit` fixture + its six pinned parser tests are **deferred** (no real device file was available; the FIT parser ships with its failure-contract tests), and the literal `samples` badge needs a dense per-second file — the 4-point TCX fixture reaches `mixed`. Next feature phase: **Phase 20** — wellness metrics (**migration required**, Sr. Dev gate). **Phase 12** — Authentication & Authorization — remains deferred and **approval-gated** (see Open decisions). Phases 8–11 and 13–18 are complete.
