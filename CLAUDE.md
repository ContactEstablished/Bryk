...
1. **Vulnerable transitive NuGet packages in `Bryk.API`** (confirmed via `dotnet list package --vulnerable`): `Microsoft.Build` 17.11.4 (**High**) and `NuGet.Packaging`/`NuGet.Protocol` 6.12.1 (Low) come in through the `Microsoft.VisualStudio.Web.CodeGeneration.Design` reference — a design-time scaffolding tool; consider dropping it. `System.Security.Cryptography.Xml` 9.0.0 (**High**) flows in from `Bryk.Infrastructure`.
