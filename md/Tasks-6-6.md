...
## Goal
Close Phase 6 by recording the two outstanding decisions as ADRs, formalizing the `md/decisions/` folder, updating `CLAUDE.md`'s pending-decisions section, and writing the dated Phase 6 handoff. Both decisions gate Phase 7 — Mesocycle vs TrainingPlan drives the data model; Coaches as a first-class user type drives the parity-doc tagging and any coach-facing scope.

## Current code/status
- `md/decisions/` does **not** yet exist. `ls md/` today shows only `handoffs/` and `product/`. This task creates `md/decisions/`.
...
- `md/handoffs/` contains `2026-04-29-phase-4-complete.md` and `2026-05-25-phase-5-vue-onboarding-wizard.md`. Phase 6 handoff slots in alongside, dated when written.
...
- `md/product/feature-parity-trainingpeaks.md` carries coach-tagged features. The Coaches ADR decision (v1 / v2 / out-of-scope) drives the tag sweep in that doc.

## Acceptance criteria
- **`md/decisions/` folder created** with a short README at `md/decisions/README.md` explaining: ADR numbering (incrementing `NNNN-kebab-title.md`), template (minimal — Context / Decision / Consequences / Status / Date), and the rule that ADRs are immutable after merge (supersession via new ADRs, not edits).
- **ADR-0001 — Mesocycle vs TrainingPlan.** File at `md/decisions/0001-mesocycle-vs-trainingplan.md`. Captures:
...
- **ADR-0002 — Coaches as a first-class user type.** File at `md/decisions/0002-coaches-first-class-user.md`. Captures:
  - Context: the parity doc (`md/product/feature-parity-trainingpeaks.md`) lists several coach-dependent features (dashboard, athlete roster, workout/plan libraries, group calendars). Phase 12 auth is the natural carrier if coaches ship in v1.
...
- **`CLAUDE.md` updated.** Both pending-decision sections moved out of "Pending decisions" and into a brief "Resolved decisions" pointer, or deleted entirely with a reference like "see [`md/decisions/0001-...`]" depending on Sr. Dev's preferred shape. Tech-debt list (item 3) updated if Task 6-4 resolved the `MesocycleService` layer fix.
- **`md/product/feature-parity-trainingpeaks.md` tag sweep.** Coach-tagged features re-tagged per ADR-0002's decision (v1 → in-scope; v2 → deferred; out-of-scope → removed or tagged `out-of-scope`). One commit, scoped to that doc.
- **Phase 6 handoff written.** New file at `md/handoffs/YYYY-MM-DD-phase-6-complete.md` (date set when written) capturing:
...
- `md/decisions/README.md` (new) — ADR conventions for this repo.
- `md/decisions/0001-mesocycle-vs-trainingplan.md` (new).
- `md/decisions/0002-coaches-first-class-user.md` (new).
- `md/handoffs/YYYY-MM-DD-phase-6-complete.md` (new).
...
- `md/product/feature-parity-trainingpeaks.md` — coach-tagged features re-tagged per ADR-0002.

## What NOT to modify
- Do not write Phase 7 code, prompts, or migration plans. The ADR records the decision; enacting it is Phase 7's job.
- Do not edit ADRs after sign-off. Supersession-by-new-ADR is the convention; document this in `md/decisions/README.md` and follow it from the first commit.
...
## Test plan
1. Verify both ADR files render cleanly as Markdown (preview locally; check the template matches `md/decisions/README.md`'s specification).
